"""
backtest_viz/report.py — 回測統計摘要報告
"""
...
class BacktestReport:
    ...
    def print_summary(self, results: list[SymbolResult]) -> None:
        ...
        line = "─" * 72
        print(f"\n{'═'*72}")
        print(f"  AlphaGPT 回測報告")
        print(f"  公式：{self.formula}")
        if self.formula_readable:
            print(f"  解讀：{self.formula_readable}")
        print(f"{'═'*72}")
        ...
        for r in results:
            ...
            print(f"  {'總 PnL (log ret)':20s}: {r.total_return:+.6f}")
            pl = r.profit_loss_ratio
            print(f"  {'盈虧比':20s}: {pl:.4f}" if pl is not None else f"  {'盈虧比':20s}: —")
            print(f"  {'總交易筆數':20s}: {r.n_trades}")
            print(f"  {'勝率':20s}: {r.win_rate:.1%}")
            print(f"  {'平均持倉 (bars)':20s}: {r.avg_hold_bars:.1f}")
            ...
            if r.trades:
                ...
                for t in sorted_trades[:5]:
                    ...
                    print(
                        ...
                        f"進 {_ts(t.entry_time)} @ {t.entry_price:.5f}  "
